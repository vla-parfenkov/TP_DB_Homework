...
public class UserDAO {
    ...
    public User createUser(String nickname, String email, String fullname, String about) {
        template.update(con -> {
            PreparedStatement pst = con.prepareStatement(
                    "insert into user_account(nickname, email, fullname, about, forums)" + " values(?,?,?,?,ARRAY[0])",
    ...
    private static final RowMapper<User> USER_MAPPER = (res, num) -> {
        ...
        User user = new User(res.getString("about"), res.getString("email"), res.getString("fullname"),
    ...
    public List<User> getUserByForum (Integer forumId, Integer limit, String since, Boolean desc) {
        if (desc == null) {
            desc = false;
        }
